...
def get_train_transforms(img_size,img_mean,img_std):
    train_transforms = A.Compose([
                        A.RandomResizedCrop(height=img_size, width=img_size, scale=(0.25, 1.0), ratio=(0.75, 1.3333333333333333), interpolation=1, p=1.0),
...
if __name__ == '__main__':
    ...
    for fold, (train_ids,test_ids) in enumerate(kfold.split(np.zeros(len(df)),y=df['label_encoded'])):
        ...
        CHECKPOINT = f'./models/{model_architecture}_covidx_severity_baseline_weak_fold{fold}.pth'
    ...
    df.to_csv(f'./logs/{model_architecture}_covidx_severity_baseline_weak_cvmetrics.csv')
    #pd.DataFrame.from_dict([train_ids_dict,test_ids_dict],orient='columns').to_csv('./logs/cv_splits.csv')