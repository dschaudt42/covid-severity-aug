...
if __name__ == '__main__':
    ...
    for fold, (train_ids,test_ids) in enumerate(kfold.split(np.zeros(len(df)),y=df['label_encoded'])):
        ...
        train_dataset = Dataset(
            df.loc[train_ids],
            augmentation=get_train_transforms(img_size,img_mean,img_std), 
            minority_augmentation=get_minority_transforms(img_size,img_mean,img_std),
        )
        ...
        CHECKPOINT = f'./models/{model_architecture}_covidx_severity_minaugv2_fold{fold}.pth'
    ...
    df.to_csv(f'./logs/{model_architecture}_covidx_severity_minaugv2_cvmetrics.csv')
